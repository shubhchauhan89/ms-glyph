...
$total_result = mysqli_query($con, "SELECT COUNT(*) AS cnt FROM blog");
$total_row    = mysqli_fetch_assoc($total_result);
$total_posts  = (int)($total_row['cnt'] ?? 0);
$total_pages  = (int)ceil($total_posts / $posts_per_page);

$blog = mysqli_query($con, "SELECT * FROM blog ORDER BY id DESC LIMIT $posts_per_page OFFSET $offset");
?>
        ...
        <section class="news-section section-padding fix">
            ...
            while ($row = mysqli_fetch_array($blog)) {
                $description = strip_tags($row['descrip']); // Strip HTML tags for clean excerpt
            
                $maxLength = 110; // Maximum length of the excerpt
                $descrip = strlen($description) > $maxLength ? substr($description, 0, $maxLength) . '...' : $description;
                ...
                // Get day, month, and year from the timestamp
                ...
                $new_string = ' ';
                $name = $row['category'];
                $category = str_replace($old_string, $new_string, $name);
                ?>
