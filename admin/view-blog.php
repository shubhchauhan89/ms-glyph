...
if(isset($_POST['delete'])) {
    ...
    if($p) {
        echo "<script>alert('Selected blogs deleted successfully');</script>";
    } else {
        echo "<script>alert('Error occurred while deleting blogs');</script>";
    }
    echo "<script>window.location.href = 'view-blog.php'</script>";
}

$resultt = mysqli_query($con, "SELECT * FROM blog ORDER BY id DESC");
?>
                    ...
                    <div class="card-body">
                        <form method="post">
                            <table id="datatable" class="table table-bordered dt-responsive nowrap" style="border-collapse: collapse; border-spacing: 0; width: 100%;">
                                ...
                                <tbody>
                                    <?php $index = 1; while($roww = mysqli_fetch_array($resultt)) { ?>
                                    <tr>
                                        <td><?php echo $index++; ?></td>
                                        <td><input type="checkbox" name="ids[]" value="<?php echo $roww['id']; ?>"></td>
                                        <td><img style="width:50px;" src="assets/images/blog/<?php echo $roww["img"]; ?>"></td>
                                        <td><?php echo $roww["title"]; ?></td>
                                        <td><?php echo $roww["category"]; ?></td>
                                        <td><?php echo substr(strip_tags($roww['descrip']), 0, 50) . '..'; ?></td>
                                        <td><a href="add-blog.php?edit=<?php echo $roww["id"]; ?>" class="btn btn-success waves-effect waves-light text-white">Edit</a></td>
                                        <td><a href="view-blog.php?delete_id=<?php echo $roww["id"]; ?>" onclick="return confirm('Are you sure?')" class="btn btn-danger waves-effect waves-light text-white">Delete</a></td>
                                    </tr>
                                    <?php } ?>
                                </tbody>
                            </table>
